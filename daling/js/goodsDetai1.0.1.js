...
$(function(){
	...
	//判断是哪一个JSON
	//今日上新
	if(goodsId>=100 && goodsId<110){
		ajax("json/todayNew-goods-01.json",goodsId);
	}
	else if(goodsId>=200 && goodsId<210){
		ajax("json/todayNew-goods-02.json",goodsId);
	}
	//大家都说好
	else if(goodsId>=110 && goodsId<120){
		ajax("json/allSayGood-goods-00.json",goodsId);
	}
	else if(goodsId>=210 && goodsId<220){
		ajax("json/allSayGood-goods-01.json",goodsId);
	}
	else if(goodsId>=220 && goodsId<230){
		ajax("json/allSayGood-goods-02.json",goodsId);
	}
	//新品黑马
	else if(goodsId>=120 && goodsId<130){
		ajax("json/newBlack-goods.json",goodsId);
	}
	//今日特惠
	else if(goodsId>=150 && goodsId<180){
		ajax("json/todayDiscount-goods-01.json",goodsId);
	}
	else if(goodsId>=180 && goodsId<200){
		ajax("json/todayDiscount-goods-02.json",goodsId);
	}
	...
	//进入购物车页面
	$(".goods .goods-detail .goods-detail-tab .buyNow").click(function(){
	...
	function fly1(){  
		$(".goods .goods-detail .goods-detail-tab .intoCart").click(function(e){
			...
			//结束的位置
//			var endOffset = $(".sidebar-list1 .sidebar-cart").offset();
...
//			});
	...
	function addCart(goods){
		$(".goods .goods-detail .goods-detail-tab .intoCart").click(function(){
			...
			var goodsCount = parseInt( $(".goods .goods-detail .goods-detail-tab .buyCount").val() );
